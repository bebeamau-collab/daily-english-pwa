...
這是一個完全不需要後端、帳號或 API 的手機優先英文學習 APP。每天依日期固定選出 10 個不重複單字，提供 KK 音標與可選女聲／男聲的美式發音，完成後依 1、3、7、14、30 天安排翻卡複習；所有進度只存在自己的裝置瀏覽器中。
...
每張今日單字卡與複習卡都有 KK 音標。點喇叭按鈕會使用裝置內建的美式英文語音，不需要串接網路 API；上方可以切換女聲或男聲，選擇會自動保存。不同 iPhone、iPad 或瀏覽器內建的語音名稱不完全相同，若沒有指定性別的美式聲線，APP 會改用裝置上可用的美式英文聲音。離線發音是否可用，取決於該聲音是否已下載到裝置。
...
測試會檢查 300 筆字庫與音標、美式男女聲選擇、固定選字不重複、完成學習、翻卡排程、記得／忘記、精通、streak 與 JSON 持久化。

## 自己新增單字

打開 `words.js`，依檔案最上方註解的格式新增一行即可。英文例句中要原樣包含該單字或片語，程式才會自動將它粗體顯示；也請在 `phonetics.js` 加入相同英文鍵值的 KK 音標。新增後也要把 `service-worker.js` 的 `CACHE_NAME` 改成新版本（例如 `daily-english-v3`），已安裝的手機才會更快更新。
