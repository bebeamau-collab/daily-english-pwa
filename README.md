...
- `styles.css`：手機優先樣式、淺色／深色閱讀模式與安全區域
- `app.js`：畫面互動與資料保存
...
APP 預設使用明亮的淺色底，白天在手機或平板上閱讀文章與單字會更清楚。頁面上方的「閱讀底色」可隨時切換「淺色／深色」；選擇會保存在目前裝置，關閉或重新開啟 APP 後仍會沿用。這個選擇不會影響單字、複習排程或其他學習紀錄。
...
語音使用 Apache 2.0 授權的 [Kokoro-82M](https://huggingface.co/hexgrad/Kokoro-82M) 預先產生，不需要在手機中放 API 金鑰。Bella、Michael 的單字、例句與文章會以原始 1.0 倍速及原音高播放，避免瀏覽器時間伸縮造成破音；音量套用 1.55 倍基礎增益及動態峰值保護，讓發音更清楚又避免突然爆音。音檔第一次播放需要網路，成功播放後 service worker 會保存該音檔，之後可離線重播；如果自然語音檔暫時無法取得，APP 才會改用裝置內建的美式英文語音，備用速度維持原本的 0.86 倍，而且不會再用變更音高的方式模擬性別。
...
測試會檢查 300 筆字庫、600 組例句與音標、30 篇連貫故事及中英文單字覆蓋、Bella／Michael 音檔路徑、固定選字不重複、完成學習、翻卡排程、記得／忘記、精通、streak，以及閱讀底色與其他設定的 JSON 持久化。
